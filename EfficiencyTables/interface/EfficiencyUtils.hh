#ifndef HCC_UTILS_EFFICIENCYUTILS_H
#define HCC_UTILS_EFFICIENCYUTILS_H
...
namespace hcc
{
  ...
  TGraphAsymmErrors* createEfficiencyGraph(TH1F* numerator, TH1F* denominator,
    ...
    TH1F *d = denominator;  
    if (bins.size() > 0) {   
      n = hcc::rebin(numerator,bins);
      d = hcc::rebin(denominator,bins);
    }
    ...
    for (int b=0; b<nbins ; ++b) {
      ...
      xErr[b] = (n->GetXaxis()->GetBinWidth(b+1))/2;  //0.0
  ...
  TH2F* createEfficiencyHist2D(TH2F* numerator, TH2F* denominator,
    ...
    if (xbins.size() > 0 && ybins.size() > 0) {
      n = hcc::rebin(numerator,xbins,ybins);
      d = hcc::rebin(denominator,xbins,ybins);
    }
